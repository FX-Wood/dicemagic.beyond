var title, customRollButton, rollTextInput, resultsBox;
// default color
var popupThemeColor = '#c53131'

// get stored theme color, if any
getThemeAndLastRollFromStorage = async () => {
    const store = await new Promise(resolve => chrome.storage.sync.get(['themeColor', 'lastCustomRoll'], resolve))
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    rollTextInput.addEventListener('focus', (e) => e.currentTarget.setSelectionRange(0, e.currentTarget.value.length))
    rollTextInput.addEventListener('click', (e) => e.currentTarget.setSelectionRange(0, e.currentTarget.value.length))
})
...
function renderRoll(data) {
    ...
    const resultCharWidth = rolls.reduce((max, next) => {
        console.log('max', max, 'next', next)
        let [ raw, result ] = next.split(' = ')
        console.log('raw', raw, 'result', result)
        if (result) {
            result = result.replace(/\*/g, '')
            console.log('result', result, 'length', result.length, 'max', max)
